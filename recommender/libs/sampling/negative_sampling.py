...
class NegativeSampling(object):
    ...
    def format_dataset(self) -> Dict[str, torch.Tensor]:
        ...
        if self.is_triplet:
            ...
            return {
                ModelForwardArgument.USER_IDX: torch.tensor(user_ids).to(self.device),
                ModelForwardArgument.POS_ITEM_IDX: torch.tensor(pos_item_ids).to(
                    self.device
                ),
                ModelForwardArgument.NEG_ITEM_IDX: torch.tensor(neg_item_ids).to(
                    self.device
                ),
                ModelForwardArgument.Y: torch.zeros(size=(len(user_ids),)).to(
                    self.device
                ),  # dummy y value
            }
        else:
            ...
            return {
                ModelForwardArgument.USER_IDX: torch.tensor(user_ids).to(self.device),
                ModelForwardArgument.ITEM_IDX: torch.tensor(item_ids).to(self.device),
                ModelForwardArgument.Y: torch.tensor(y).to(self.device),
            }
